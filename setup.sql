...
CREATE TABLE Subject(
    Subject_ID INTEGER PRIMARY KEY NOT NULL CHECK (LENGTH(Subject_ID) = 8),
...
CREATE TABLE Professor(
    ...
    Professor_Gender TEXT NOT NULL CHECK (Professor_Gender = 'Male' OR Professor_Gender = 'Female'),
...
CREATE TABLE ProfessorEmail(
    ProfessorEmail_ID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
    Professor_Email TEXT NOT NULL,
    Professor_ID INTEGER NOT NULL,
    FOREIGN KEY (Professor_ID) 
        REFERENCES Professor (Professor_ID) ON UPDATE CASCADE
);
...
CREATE TABLE Student(
    ...
    Student_Gender TEXT NOT NULL CHECK (Student_Gender = 'Male' OR Student_Gender = 'Female'),
...
CREATE TRIGGER StudentEmail_AutoGenerate
    AFTER INSERT ON Student
    BEGIN
        INSERT INTO StudentEmail (Student_Email, Student_ID) VALUES (NEW.Student_ID || '@go.buu.ac.th', NEW.Student_ID);
        INSERT INTO StudentEmail (Student_Email, Student_ID) VALUES (NEW.Student_ID || '@my.buu.ac.th', NEW.Student_ID);
    END;
...
CREATE TABLE ClassAttendance(
    ...
    Professor_Check INTEGER NOT NULL CHECK (Professor_Check = 1 OR Professor_Check = 0), -- 0 = not check, 1 = check
    
    ClassRoomSchedule_ID INTEGER NOT NULL,
    Professor_ID INTEGER NOT NULL,

    FOREIGN KEY (ClassRoomSchedule_ID) 
        REFERENCES ClassRoomSchedule (ClassRoomSchedule_ID) ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (Professor_ID) 
        REFERENCES Professor (Professor_ID) ON UPDATE CASCADE
);

-- create student attendance table
CREATE TABLE StudentAttendant(
    ...
    Student_Check INTEGER NOT NULL CHECK (Student_Check = 1 OR Student_Check = 0) , -- 0 = not check, 1 = check

    FOREIGN KEY (ClassAttendance_ID) 
        REFERENCES ClassAttendance (ClassAttendance_ID) ON DELETE CASCADE ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (Student_ID) 
...
CREATE TABLE Employee(
    ...
    Employee_Gender TEXT NOT NULL CHECK (Employee_Gender = 'Male' OR Employee_Gender = 'Female'),
...
CREATE TABLE WorkAttendance(
    ...
    WorkerAttendance_check_in INTEGER NOT NULL CHECK (WorkerAttendance_check_in = 0 OR WorkerAttendance_check_in = 1), -- 0 = not check, 1 = check

    WorkAttendance_time_out TEXT NULL, -- example '10:00'
    WorkerAttendance_check_out INTEGER NOT NULL CHECK (WorkerAttendance_check_out = 0 OR WorkerAttendance_check_out = 1), -- 0 = not check, 1 = check
